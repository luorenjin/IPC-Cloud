 ...
 * 网络设置区块（net.* 五键）。
 *
 * 与页面其它配置分开提交，原因是后果量级不同：改错亮度最多是画面难看，
 * 改错 IP/掩码/网关会让设备从平台上失联，且这五个键在固件规则表里都是 reboot_required，
 * 「保存」与「生效」还隔着一次重启。
 *
 * 但**保存按钮不在本区块里**：它和页面级「保存并下发」打的是同一个端点，
 * 两颗同款主色按钮上下相邻会被读成“重复的两次保存”。所以这里只负责渲染与就地校验，
 * 下发动作与强确认都在页面保存栏（见 pages/devices/[id].vue 的保存栏注释）。
 */
...
const emit = defineEmits<{ change: [key: string, value: any]; invalid: [v: boolean] }>()
const { t } = useI18n()
...
// 校验状态上报给页面：提交按钮在页面保存栏，它需要知道网络字段当前能不能下发。
// 初始值在 onMounted 里补发一次——万一设备上存的本来就是非法值，watch 不会有“变化”可触发。
watch(hasError, (v) => emit('invalid', v))
onMounted(() => emit('invalid', hasError.value))
</script>
