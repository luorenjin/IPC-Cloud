 ...
 * handler 应**先**用 http_respond_* 把响应入队，**再**调用本函数登记后续
 * 动作——http_respond_* 只是把数据拷进该连接的发送队列，真正的 send() 只
 * 发生在事件循环的可写分支（conn_flush_send）里，调用 http_respond_* 之后
 * 数据不一定已经发出去。
 *
 * **调用顺序由返回值强制**：登记时如果该连接此刻已经没有待发数据
 * （c->slen==c->soff——可能是还没调用过 http_respond_*，也可能是响应小到
 * 上一次可写事件已经连它一起发完了），返回 HAL_ESTATE，且**不会**注册
 * 回调、也**不会**同步执行它。这是有意的设计：本函数不替调用方猜"既然没
 * 数据在排队，大概是想让它立刻执行"，而是把调用顺序颠倒这个编程错误变成
 * 一个响亮的错误返回值，而不是静默地把重启/恢复出厂提前到响应入队之前
 * 执行——那正是这个函数存在的全部理由。调用方必须检查返回值。
 *
 ...
 *   2) 不可阻塞——与 handler 同一条非阻塞纪律（不做磁盘 I/O、不做网络阻塞
 *      调用）。**唯一豁免**：回调本身就是"以重启/关机收尾的终态动作"
 *      （执行完必然导致进程/事件循环随后立即终止，不再服务任何后续
 *      请求）——这种情况下阻塞事件循环不再有"饿死其他连接"的代价，
 *      该规则本来要保护的东西已经不成立。这条豁免不覆盖第 1、3 条，
 *      也不覆盖"之后还会继续服务请求"的一般回调；
