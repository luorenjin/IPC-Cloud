# 固件 Docker 编译环境（GOKE GK7205V200 IPC Linux SDK）
...
`firmware/` 目前只有 `platform/mock`（x86 模拟平台），CLAUDE.md 明确要求**芯片选型定稿前不得向 core/modules 添加芯片相关假设**。决策记录中 GK7205V200 是"量产倾向"而非最终定稿，但硬件团队已经在用这颗芯片做打样（`D:/JetsCam/Source/国科微/` 下已备有原厂 SDK、原理图与开发板资料）。

本次目标**不是**给 `firmware/` 接入 gk7205v200 平台实现（那属于芯片定稿后的工作），而是**先把 GOKE 官方 SDK 本身的编译环境用 Docker 固化下来**：任何人在任何一台 Windows 机器上，不装 Ubuntu、不碰 WSL 里的全局环境，就能可重复地把 SDK 编译出 uboot/kernel/rootfs 镜像，为后续硬件调试和固件预研做准备。

### 非目标

- 不实现 `firmware/platform/gk7205v200/`（芯片未定稿，CLAUDE.md 禁止）。
- 不把 `firmware/` 通用层接入这套交叉编译环境（留作芯片定稿后的独立任务）。
- 不做多芯片矩阵编译（GK7205V300/GK7605V100/GK7202V300 等）——本次只打通 GK7205V200，其余型号复用同一镜像时按需扩展 `CHIP` 变量即可，不在本次实现范围内。
...
- **多芯片支持**：SPC030 的 `configs/` 下有 `gk7202v300/ gk7205v200/ gk7205v300/ gk7605v100/` 四套配置，本次只用 `gk7205v200`。
...
- **磁盘**：D 盘剩余 90GB，够用（源码卷 ~2GB + 编译产物数 GB）。
...
- **产物导出**：`make build` 的产物（`out/gk7205v200/image/` 下的 uboot/kernel/rootfs 镜像）留在卷里，`make sdk-export` 显式把这些文件 `cp` 到 bind mount 至 Windows 侧的 `firmware/docker/out/`，供后续用 PC 端烧录工具或 boot 命令行使用。这一步把"容器内产物"和"宿主可见文件"的边界明确分开，避免误以为容器里所有文件都能直接在资源管理器里看到。
...
| `sdk-export` | 把卷内 `out/$(CHIP)/image/` 拷到 bind mount 的 `/host-out/`（对应宿主 `firmware/docker/out/`） |
| `sdk-clean` | 容器内 `make <target>_clean`；`sdk-clean-all` 额外删卷（需二次确认） |

`CHIP` 默认 `gk7205v200`，可覆盖（如 `make sdk-build CHIP=gk7205v300`）——不在 Makefile/Dockerfile 里硬编码单一芯片型号，呼应 CLAUDE.md「芯片选型未定稿」的约束；但本次只验证 `gk7205v200` 这一条路径。
...
`firmware/` 通用层的 CMake 门禁明确禁止 core/modules 出现平台宏；`platform/gk7205v200/` 目录本身还不存在，写它需要芯片选型定稿。本次范围收窄为"把 GOKE 原厂 SDK 编译环境跑通"，为后续硬件调试提供确定性环境，firmware/ 与该环境的对接是芯片定稿后的下一个独立任务。
...
3. `make sdk-build CHIP=gk7205v200` 跑完无 `FATAL`/`Error 1` 类致命错误，产出 `out/gk7205v200/image/` 下的 uboot/kernel/rootfs 镜像文件。
4. `make sdk-export` 后，`firmware/docker/out/` 在 Windows 侧可见对应镜像文件，文件大小非零。
5. 重跑 `make sdk-build`（不删卷）走增量编译，不报错。

---

## 7. 已知限制 / 后续工作

- 本次只验证 `gk7205v200` 一条链路；`gk7202v300`/`gk7205v300`/`gk7605v100` 的 `CHIP` 切换未做端到端验证。
- 没有实际开发板可烧录验证镜像能否真正跑通；本次止步于"编译产出 image 文件"。
- `firmware/` 通用层接入 `platform/gk7205v200/` 是明确的后续工作，需等待 `Docs/PRD/决策记录与待定事项.md` 中芯片选型定稿。
